...
## 後續驗證：座位排別與座位圖（2026-09-18）

- 活動類型決定排別上限：音樂演唱會 50 排（AX）、舞台劇 100 排（CV）、展覽特展 150 排（ET）；後端由 A 排生成至管理員選取的最後一排，每排 1～10 席。
- `AdminActivityImageTests` 由 4 項增至 6 項，新增活動類型排數上限與超出上限拒絕的單元測試。後端 `mvnw.cmd -q test` 共 24 項通過（5 項 Spring／權限、13 項訂單、6 項活動／座位配置）；前端 `npm run build` 通過。
- `SeatMap.vue` 改為逐排顯示，提供快速跳排、座位區捲動、狀態圖例及載入失敗重試；訂票座位 API 讀取排別時會去除前置空白。
- 本次未執行瀏覽器座位圖測試或真實 PostgreSQL 座位配置寫入／修改測試。24 項 Java 測試中的座位配置案例使用 mock Mapper；既有 Microsoft Edge 9 項與 Python 6 項仍為 2026-09-10 紀錄。
