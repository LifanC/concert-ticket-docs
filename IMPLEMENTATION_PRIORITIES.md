...
已修正付款／取消先更新本人指定場次的待付款訂單，再更新庫存；重要更新失敗會拋出業務例外回滾。訂位增加容量條件，逾期改由獨立交易 Service 執行並修正到期判斷；通知與排程安排在交易提交後執行。取消保留原購票數量，不修改固定容量。

已通過 10 項真實 PostgreSQL 交易測試，包含重送、所有權、到期、庫存上限與回滾。這只代表第一批一致性修正完成，不代表以下所有優先項目或 Phase 3 已完成。

### 第二批實作進度（2026-09-08）

沿用原本依賴版本與 `db-init` 初始化方式，將訂位完整性結構整合至 `db-init/init.sql`。已接上場次座位模型、有效占用唯一限制、資料庫庫存約束、集中式付款／取消／逾期狀態轉換、資料庫逾期補償掃描，以及建立訂單的 `Idempotency-Key`。前端已同步依場次查詢不可用座位與保留重試 key；後台更新場次不再將已售數歸零。

共 19 項真實 PostgreSQL 交易／併發測試通過，包含 100 個請求搶同一座位恰好一筆成功；前端建置通過。初始化、既有資料升級與重跑測試方式見 [db-init/README.md](db-init/README.md)。退款、完整 HTTP／Security／E2E、CI、訂單編號邊界與 Phase 3 仍待完成。
