...
沿用原本依賴版本與 `db-init` 初始化方式，新增 `zz-booking-integrity.sql`。已接上場次座位模型、有效占用唯一限制、資料庫庫存約束、集中式付款／取消／逾期狀態轉換、資料庫逾期補償掃描，以及建立訂單的 `Idempotency-Key`。前端已同步依場次查詢不可用座位與保留重試 key；後台更新場次不再將已售數歸零。
